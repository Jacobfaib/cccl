...
//! Strong and weak scaling of `cudax::sort` over a growing set of GPUs, across MPI processes.
//!
//! This is the sort counterpart of `reduce_mpi.cu`, and differs from it only in the algorithm
//! under test.
//!
...
//!
//! Each rank allocates its input in the default memory pool of its own locality domain. Unlike
//! the reduction, which sends one element per rank, the sort exchanges the keys themselves, so
//! the volume that crosses domains falls as the rank count grows.
...
//! Elements one rank sorts. "Elements" is the total under strong scaling and the per-rank size
//! under weak scaling, so only this differs between the two.
...
void add_summary(nvbench::state& state, cuda::std::size_t per_rank_elements, cuda::std::size_t num_ranks)
{
  ...
  ranks.set_string("description", "Locality domains taking part in the sort");
  ...
  per_rank.set_string("description", "Elements sorted by each rank");
  ...
  devices.set_string("description", "Devices taking part in the sort");
  devices.set_int64("value", static_cast<cuda::std::int64_t>(BENCHMARK_MPI_SIZE));
}

//! One rank per locality domain of this process's device. A rank sorts memory that is local to
//! its own domain, so the local sort never crosses a domain boundary.
...
template <class T>
void sort(nvbench::state& state, nvbench::type_list<T>)
{
  ...
  // Each rank allocates in the pool of its own domain, so its local sort reads only memory that
  // is local to that domain.
  ...
  for (auto&& comm : comms)
  {
    ...
    in.push_back(cuda::make_buffer<T>(s, pool, per_rank, cuda::no_init));
...
NVBENCH_BENCH_TYPES(sort, NVBENCH_TYPE_AXES(element_types))
  .set_name("sort_mpi")
