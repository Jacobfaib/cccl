...
//! value unchanged. For example, for integers/foats and plus, the identity element is 0. For
//! maximum and minimum, the identity values are INT_MIN, and INT_MAX respectively.
//!
  ...
  for (auto&& [__comm, __env, __inputs] : ::cuda::std::ranges::views::zip(__comms, __envs, __range_of_inputs))
  {
    __partials.emplace_back(
      ::cuda::experimental::__detail::__reduce::__local_reduction<typename __partial_type::__buffer_type>(
        /*__ROOT_RANK=*/0, __comm, __env, __inputs, __init, __ident, __op));
  }
