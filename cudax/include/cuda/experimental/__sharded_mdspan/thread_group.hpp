...
public:
  ...
  [[nodiscard]] constexpr rank_type rank() const noexcept
  {
    return __rank_;
  }
  ...
  [[nodiscard]] constexpr ::cuda::std::span<const logical_device> devices() const noexcept
  {
    return __devices_;
  }

  [[nodiscard]] static constexpr ::cuda::std::size_t required_shared_memory_size(size_type __group_size) noexcept
  {
    return sizeof(__shared_data) + alignof(__shared_data) - 1 + required_scratch_mem_size_(__group_size);
  }

  static constexpr void* initialize_shared_memory(void* __mem) noexcept
  {
...
private:
  [[nodiscard]] static constexpr ::cuda::std::size_t required_scratch_mem_size_(size_type __group_size) noexcept
  {
    // All members of the group have a void*'s worth of private memory
    return __group_size * sizeof(void*);
  }

  [[nodiscard]] static constexpr ::cuda::std::size_t required_scratch_mem_alignment_() noexcept
  {
    return alignof(::cuda::std::max_align_t);
  }

  [[nodiscard]] __shared_data& shared_data_() noexcept
  {
    return reinterpret_cast<__shared_data&>(*__shared_mem_.get());
  }

  [[nodiscard]] const __shared_data& shared_data_() const noexcept
  {
    return reinterpret_cast<const __shared_data&>(*__shared_mem_.get());
  }

  [[nodiscard]] __shared_data& mut_shared_data_() const noexcept
  {
