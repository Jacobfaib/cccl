...
#ifndef _CUDA_STD___RANGES_VARIANT_LIKE_H
#define _CUDA_STD___RANGES_VARIANT_LIKE_H
...
_CCCL_CONCEPT __is_trivially_swappable_v =
  ...
  && !::cuda::std::ranges::__swap::__unqualified_swappable_with<_Tp, _Tp>;
...
struct __vl_destruct_base
{
  ...
  _CCCL_API constexpr __vl_destruct_base() noexcept(is_nothrow_default_constructible_v<_Tp2>)
      : __first_()
      , __contains_{__variant_like_state::__holds_first}
  {}

  _CCCL_API constexpr explicit __vl_destruct_base(__variant_like_state __state) noexcept
      : __contains_{__state}
  {}

  _CCCL_EXEC_CHECK_DISABLE
  template <class... _Types>
  _CCCL_API constexpr explicit __vl_destruct_base(__construct_first, _Types&&... _Args) noexcept(
    is_nothrow_constructible_v<_Tp, _Types...>)
      : __first_{::cuda::std::forward<_Types>(_Args)...}
      , __contains_{__variant_like_state::__holds_first}
  {}

  _CCCL_EXEC_CHECK_DISABLE
  _CCCL_TEMPLATE(class... _Types)
  _CCCL_REQUIRES((!same_as<_Tp, _Up>) )
  _CCCL_API constexpr explicit __vl_destruct_base(__construct_second, _Types&&... _Args) noexcept(
    is_nothrow_constructible_v<_Up, _Types...>)
      : __second_{::cuda::std::forward<_Types>(_Args)...}
...
public:
      ...
      : __base{__other.__contains_}
  {
    ...
    {
      case __variant_like_state::__holds_first:
        ::cuda::std::__construct_at(::cuda::std::addressof(this->__first_), __other.__first_);
        break;
      case __variant_like_state::__holds_second:
        ::cuda::std::__construct_at(::cuda::std::addressof(this->__second_), __other.__second_);
        break;
      case __variant_like_state::__nothing:
        break;
    }
  ...
  _CCCL_API _CCCL_CONSTEXPR_CXX20 __variant_like(const __variant_like<_OTp, _OUp>& __other) noexcept(
      ...
      : __base{__other.__contains_}
  {
    ...
    {
      case __variant_like_state::__holds_first:
        ::cuda::std::__construct_at(::cuda::std::addressof(this->__first_), ::cuda::std::move(__other.__first_));
        break;
      case __variant_like_state::__holds_second:
        ::cuda::std::__construct_at(::cuda::std::addressof(this->__second_), ::cuda::std::move(__other.__second_));
        break;
      case __variant_like_state::__nothing:
        break;
    }
    ...
    && is_nothrow_move_assignable_v<_Up>)
  {
    if (this->__contains_ == __other.__contains_)
    {
      ...
      {
        case __variant_like_state::__holds_first:
          this->__first_ = ::cuda::std::move(__other.__first_);
          break;
        case __variant_like_state::__holds_second:
          this->__second_ = ::cuda::std::move(__other.__second_);
          break;
        case __variant_like_state::__nothing:
          break;
      }
  ...
  _CCCL_TEMPLATE(class _OTp, class _OUp)
  _CCCL_REQUIRES((!same_as<__variant_like<_OTp, _OUp>, __variant_like>) )
  _CCCL_API _CCCL_CONSTEXPR_CXX20 __variant_like& operator=(const __variant_like<_OTp, _OUp>& __other) noexcept(
    is_nothrow_constructible_v<_Tp, const _OTp&> && is_nothrow_constructible_v<_Up, const _OUp&>
...
#endif // _CUDA_STD___RANGES_VARIANT_LIKE_H
