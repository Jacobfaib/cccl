...
#if TEST_COMPILER(GCC, <, 9) // Old gcc has issues establishing the conversion sequence
  TEST_FUNC constexpr convertible_forward_sized_iterator(forward_sized_iterator<Base> it)
      : it_(it.it_)
  {}
...
struct convertible_sized_sentinel
{
  ...
  TEST_FUNC friend constexpr bool operator==(const convertible_sized_sentinel& s, const U& base)
  {
    return s.base_ == base;
  }
  ...
  template <class U,
            cuda::std::enable_if_t<cuda::std::convertible_to<Base, U> || cuda::std::convertible_to<U, Base>, int> = 0>
  TEST_FUNC friend constexpr bool operator==(const U& base, const convertible_sized_sentinel& s)
  {
    return s.base_ == base;
  }
  template <class U,
  ...
  template <class U,
            cuda::std::enable_if_t<cuda::std::convertible_to<Base, U> || cuda::std::convertible_to<U, Base>, int> = 0>
  TEST_FUNC friend constexpr auto operator-(const convertible_sized_sentinel& s, const U& i)
  {
    return s.base_ - i;
  }
  template <class U,
            cuda::std::enable_if_t<cuda::std::convertible_to<Base, U> || cuda::std::convertible_to<U, Base>, int> = 0>
  TEST_FUNC friend constexpr auto operator-(const U& i, const convertible_sized_sentinel& s)
  {
    return i - s.base_;
  }
};
...
struct ConstCompatibleForwardSized : IntBufferView
{
  ...
  TEST_FUNC constexpr ConstCompatibleForwardSized(int (&b)[N])
      : IntBufferView(b)
  {}
#else
