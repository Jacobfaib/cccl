...
template <class T>
_CCCL_CONCEPT CanPreDecrement = _CCCL_REQUIRES_EXPR((T), T& t)((--t));

template <class T>
_CCCL_CONCEPT CanPostDecrement = _CCCL_REQUIRES_EXPR((T), T& t)((t--));

template <class T>
TEST_FUNC constexpr void noDecrementTest(T&& jv)
{
  auto iter = jv.begin();
...
{
  template <class T>
  TEST_FUNC constexpr cuda::std::remove_reference_t<T>&& operator()(T& x) const noexcept
  {
...
#if TEST_STD_VER >= 2020 && defined(_CCCL_BUILTIN_IS_CONSTANT_EVALUATED)
  if (!cuda::std::is_constant_evaluated())
#endif // TEST_STD_VER >= 2020 && _CCCL_BUILTIN_IS_CONSTANT_EVALUATED
  {
    // skip empty inner
