...
{
  TEST_FUNC constexpr ChildView* begin()
  {
    return nullptr;
  }
  TEST_FUNC constexpr const ChildView* begin() const
  {
    return nullptr;
  }
  TEST_FUNC constexpr const ChildView* end() const
  {
    return nullptr;
  }
};
...
{
  TEST_FUNC constexpr const ChildView* begin()
  {
    return nullptr;
  }
  TEST_FUNC constexpr const ChildView* end()
  {
    return nullptr;
  }
};
...
struct Pred
{
  TEST_FUNC constexpr ChildView operator()(int) const noexcept
  {
    return ChildView{};
  }
};
...
template <class T>
_CCCL_CONCEPT HasConstBegin = _CCCL_REQUIRES_EXPR((T), const T& t)((t.begin()));
