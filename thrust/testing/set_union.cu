...
void TestSetUnion(const size_t n)
{
  ...
  for (size_t i = 0; i < num_sizes; i++)
  {
    ...
    h_end = thrust::set_union(h_a.begin(), h_a.end(), h_b.begin(), h_b.begin() + size, h_result.begin());
...
void TestSetUnionToDiscardIterator(const size_t n)
{
  ...
  h_result = thrust::set_union(h_a.begin(), h_a.end(), h_b.begin(), h_b.end(), cuda::make_discard_iterator());
