...
struct TestReverseCopyToDiscardIterator
{
  void operator()(const size_t n)
  {
    ...
    cuda::discard_iterator h_result = thrust::reverse_copy(h_data.begin(), h_data.end(), cuda::make_discard_iterator());

    cuda::discard_iterator d_result = thrust::reverse_copy(d_data.begin(), d_data.end(), cuda::make_discard_iterator());
